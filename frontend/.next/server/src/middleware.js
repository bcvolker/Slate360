...
;// CONCATENATED MODULE: ./src/middleware/requireRole.ts
 ...
 */ async function requireRole(role, options = {}) {
    const { redirectTo = "/login", allowUnauthenticated = false, fallbackRole = "user" } = options;
    // Convert single role to array for consistent handling
    const requiredRoles = Array.isArray(role) ? role : [
        role
    ];
    try {
 ...
 */ function withRole(role, options = {}) {
 ...
 */ const requireCEO = ()=>requireRole("ceo", {
 ...
 */ const requirePremium = ()=>requireRole("user", {
        redirectTo: "/pricing",
        allowUnauthenticated: false
    });
/**
 * Middleware for enterprise tier access
 */ const requireEnterprise = ()=>requireRole("user", {
 ...
 */ function hasMinimumRole(userRole, minLevel) {
...
async function middleware(request) {
    ...
    if (pathname.startsWith("/dashboard")) {
        ...
        if (pathname === "/dashboard/ceo") {
            const ceoCheck = await requireRole("ceo", {
                redirectTo: "/unauthorized"
            });
            if (ceoCheck) return ceoCheck;
        }
        // All other dashboard routes - require basic authentication
        const authCheck = await requireRole("user", {
            redirectTo: "/login"
        });
    ...
    if (pathname.startsWith("/admin")) {
        const adminCheck = await requireRole([
            "admin",
            "ceo"
        ], {
            redirectTo: "/unauthorized"
    ...
    if (pathname.startsWith("/premium")) {
        const premiumCheck = await requireRole("user", {
            redirectTo: "/pricing"
        });
    ...
    if (pathname.startsWith("/enterprise")) {
        const enterpriseCheck = await requireRole("user", {
            redirectTo: "/contact?subject=enterprise-upgrade"
        });
